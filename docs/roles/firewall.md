...
|lxc_host               | boolean        | undef                 | Добавляет правила работы docker контейнеров.                                             |
|restart_docker_srv     | boolean        | undef (false)         | Docker будет перезапущен для применения настроек отключения управления NetFilter.        |

!!! failure
    На хостах c установленным Docker'ом динамическое управления правилами сервисом отключено. В файл `/etc/docker/daemon.json` добавлена опция `"iptables": false`.
    На проброшенные порты необходимо настроить разрешающее правило.

!!! attention
    Если при выполнение сценария служба `docker` не была перезапущена, то ее необходимо перезапустить потом отдельно для применения настроек. **ВСЕ КОНТЕЙНЕРЫ БУДУТ ПЕРЕЗАПУЩЕНЫ!!!**
